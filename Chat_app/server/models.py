...
# Model to represent categories for servers
...
class Category(models.Model):
    ...
    icon = models.ImageField(
        upload_to=category_icon_update_path,
        null=True,
        blank=True,
...
class Server(models.Model):
    ...
    member = models.ManyToManyField(settings.AUTH_USER_MODEL)  # Many-to-many relationship with users

    def __str__(self):
        return f"{self.name}-{self.id}"

# Model to represent channels within servers
class Channel(models.Model):
    ...
    # Custom save method to convert channel name to lowercase before saving
    def save(self, *args, **kwargs):
        if self.id :
            existing = get_object_or_404(Category,id=self.id)
            if existing.icon != self.icon:
